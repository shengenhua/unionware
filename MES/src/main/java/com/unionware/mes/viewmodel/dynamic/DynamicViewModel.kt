package com.unionware.mes.viewmodel.dynamic
...
@HiltViewModel
open class DynamicViewModel @Inject constructor() : BaseMESViewModel() {
    @JvmField
    @Inject
    ...
    inner class VirtualView : SingleLiveEvent<Any>() {
        ...
        private fun combineViewData(
            ...
            /* 子单据体 */
            if (data?.fSubEntryCollects.isNullOrEmpty()) {
                subEntryCollectsLiveData.postValue(mutableListOf())
                entryAndSub.subEntry = EntryAndSubBean.EntryView(false)
            } else {
                //获取子单据体的 view
                val subEntityView =
                    view?.filter { it.entity == "FSubEntryCollects" }?.associateBy { it.key }
                data?.fSubEntryCollects?.map { collects ->//多条数据
                    DynamicEntryBean().apply {
                        entryAndSub.subEntry = EntryAndSubBean.EntryView(
                            true,
                            collects.containsKey("FBarCode"),
                            collects.containsKey("FBarQty")
                        )
                        //获取 需要显示的 data
                        val showCollects = collects.filter { it.value.isVisible }
                        subEntityView?.filter {
                    ...
                    subEntryCollectsLiveData.postValue(this)
                }
            }

            /* 单据体 */
            if (data?.fEntryCollects.isNullOrEmpty()) {
                entryCollectsLiveData.postValue(mutableListOf())
                entryAndSub.entry = EntryAndSubBean.EntryView(false)
            } else {
                //获取单据体的 view
                val entityView =
                    view?.filter { it.entity == "FEntryCollects" }?.associateBy { it.key }
                data?.fEntryCollects?.map { collects ->//多条数据
                    DynamicEntryBean().apply {
                        entryAndSub.entry = EntryAndSubBean.EntryView(
                            true,
                            collects.containsKey("FBarCode"),
                            collects.containsKey("FBarQty")
                        )
