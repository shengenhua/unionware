...
import com.unionware.mes.adapter.dynamic.BottomFeatureAdapter
import com.unionware.mes.adapter.dynamic.EntryCollectsAdapter
import com.unionware.mes.adapter.dynamic.ScanBarcodeAdapter
 ...
 * Date:2024/11/19
 */
open class DynamicActivity<VM : DynamicViewModel> : BaseDynamicActivity<VM>() {
    /**
     * 单据头 适配器
     */
     ...
     * 单据体 适配器
     */
    open var entryCollectsAdapter: EntryCollectsAdapter = EntryCollectsAdapter(this)
    ...
    override fun initViewObservable() {
        ...
        mViewModel.apply {
            virtualView.apply {
                headCollectsLiveData.observe(this@DynamicActivity) { head ->
                ...
                entryOrSubLiveData.observe(this@DynamicActivity) {
                    if (it) {
                        ...
                        entryAndSubViewLiveData.value?.also { view ->
                            scanBarcodeAdapter.showUI(
                                view.entry.hasBarCode, view.entry.hasBarQty, binding?.fabAdd
                            )
                        }
                    } else {
                ...
                entryAndSubViewLiveData.observe(this@DynamicActivity) {
                ...
                entryCollectsLiveData.observe(this@DynamicActivity) {
                ...
                subEntryCollectsLiveData.observe(this@DynamicActivity) {
    ...
    override fun initView() {
        super.initView()
        mViewModel.virtualViewRequest.createView(unionware.base.model.req.ViewReq().apply {
            formId = reportRuleId
//            primaryId = primaryId
            params = mapOf(
                Pair("taskId", taskId),
        ...
        binding?.apply {
            fabAdd.setOnClickListener {
                ...
                mViewModel.virtualViewRequest.command(
                    ...
                    if (mViewModel.virtualView.entryOrSubLiveData.value == true) "FEntryCollects" else "FSubEntryCollects"
                )
            }
        }
    ...
    override fun onActionSubmitConfirm() {
        //提交
        mViewModel.virtualViewRequest.command("INVOKE_SUBMIT")
    }
    ...
    override fun tailAdapter(): RecyclerView.Adapter<RecyclerView.ViewHolder>? {/*初始化 单据体,子单单据体的 监听*/
        subEntryCollectsAdapter.apply {
            ...
            addOnItemChildClickListener(R.id.tvDelete) { _, _, position ->
                XPopup.Builder(this@DynamicActivity).asConfirm("删除", "是否确认删除？") {
                    //删除行 子单据体
                    mViewModel.virtualViewRequest.commandRow(
                        "INVOKE_DELETEENTRYROW"/*删除行*/, "FSubEntryCollects"/*子单据体*/, position
                    )
                }.show()
            }
        }
