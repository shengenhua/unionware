...
import com.unionware.mes.adapter.dynamic.JobTitleAdapter
import unionware.base.model.ViewDisplay
import com.unionware.mes.R
import com.unionware.mes.view.MESBaseActivity
import com.unionware.basicui.base.activity.BaseDetailsActivity.OnButtonClickListener
...
import com.unionware.mes.databinding.ActivityDynamicProcessBinding
 ...
 * Date:2024/11/19
 */
open class BaseDynamicActivity<VM : DynamicViewModel> :
    MESBaseActivity<ActivityDynamicProcessBinding, VM>() {

    override fun onBindVariableId(): MutableList<Pair<Int, Any>> = mutableListOf()
    override fun onBindLayout(): Int = R.layout.activity_dynamic_process
...
//           override fun canScrollVertically() =false
    ...
    protected open fun topAdapter(): RecyclerView.Adapter<ViewHolder>? {
        return JobTitleAdapter(ViewDisplay("工序", "jobName").apply {
            value = this@BaseDynamicActivity.title
        }).apply {
            onEditorActionArray { adapter, text ->
                this@BaseDynamicActivity.currentFocus
    ...
    private var subButtonClickArray: SparseArray<OnButtonClickListener>? = null
    ...
    protected fun addSubButton(
        ...
        if (subButtonClickArray == null) {
            SparseArray<OnButtonClickListener>().also { this.subButtonClickArray = it }
        }
