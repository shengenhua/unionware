...
import com.unionware.wms.inter.trans.BBDTransContract;
import com.unionware.wms.inter.trans.BBDTransPresenter;
...
/**
 * @Author : pangming
 * @Time : On 2023/6/8 10:46
 * @Description : BBDTransFragment
 */
@AndroidEntryPoint
public class BBDTransFragment extends BaseBindFragment<RepackingFragmentBinding> implements BBDTransContract.View, ScanAdapter.OnEditorActionChangeListener, OnItemChildClickListener, View.OnClickListener {

    @Inject
    BBDTransPresenter presenter;
    private TransAdapter adapter;
    private ScanConfigBean bean;

    private ActivityResultLauncher<Intent> launcher;
    private BarcodeDetailsInfoBean inBarcode = null;
    private BarcodeDetailsInfoBean outBarcode = null;
    private BarcodeDetailsInfoBean detailData = null;
    private LoadingPopupView loading;
    private PrintTemplateBean printTemplateBean;

    public static BBDTransFragment newInstance(String id, String taskId) {
        Bundle args = new Bundle();
        BBDTransFragment fragment = new BBDTransFragment();
        ...
        switch (bean.getKey()) {
            ...
            case "out":
                if ("in".equals(adapter.getKey(position - 1)) && TextUtils.isEmpty(adapter.getEditTextString(position - 1))) {
                    ToastUtil.showToastCenter("请先扫描转入箱码");
                    adapter.setFocusable(position - 1);
                    return;
                }
                if (TextUtils.isEmpty(view.getText().toString().trim())) {
                ...
                break;
            case "details":
                if ("out".equals(adapter.getKey(position - 1)) && TextUtils.isEmpty(adapter.getEditTextString(position - 1))) {
                    ToastUtil.showToastCenter("请先扫描转出箱码");
                    adapter.setFocusable(position - 1);
                    return;
                }
...
//        }
        ...
        if (outBarcode == null && TextUtils.isEmpty(adapter.getEditTextGetValue("out"))) {
            ToastUtil.showToastCenter("请扫描转出箱码");
            return;
        }
        if (outBarcode == null && !TextUtils.isEmpty(adapter.getEditTextGetValue("out"))) {
            ToastUtil.showToastCenter( "转出箱码请回车做条码解析");
            return;
        }
        if (!outBarcode.getBarCode().equals(adapter.getEditTextGetValue("out"))) {
            ToastUtil.showToastCenter("转出箱码不正确，请重新扫描");
            return;
        }
        ...
        transScanInfo.setOutBarcode(outBarcode.getBarCode());
