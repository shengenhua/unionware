...
/**
 * @Author : pangming
 * @Time : On 2023/6/8 10:52
 * @Description : BBDTransPresenter
 */

public class BBDTransPresenter extends BasePresenter<BBDTransContract.View> implements BBDTransContract.Persenter {
    PackingApi api;

    @Inject
    public BBDTransPresenter(PackingApi api) {
        this.api = api;
    }
    ...
    public void analysisPackBarcodeByTransfer(Context context, AnalysisReq req, int pos) {
        ...
        if ("in".equals(req.getPackCodeType())) {
            List<TransScanInfo> list =
                    ThreadTask.getTwo(() -> DatabaseProvider.getInstance().getTransScanInfoDao().queryByCodeAndType(req.getSetId(), "2"));
    ...
    public void confirmEntry(Context context, TransScanInfo transScanInfo) {
        ...
        if (!TextUtils.isEmpty(transScanInfo.getInBarcode())) {
            List<TransScanInfo> list =
                    ThreadTask.getTwo(() -> DatabaseProvider.getInstance().getTransScanInfoDao().queryByCodeAndType(transScanInfo.getInternalCodeId(), "2"));
            int size = list.size();
            if (size == 0) {
                ...
                transScanInfo2.setTransType("2");
                ThreadTask.getTwo(() -> {
                    return DatabaseProvider.getInstance().getTransScanInfoDao().insert(transScanInfo2);
                });
            }
        }
        //控制当前子项条码必需与已扫描的子项条码属于同一类条码，例如，若首个扫描录入的子项条码为包装条码，
        // 则后续扫描的子项条码也必需为包装条码；若首个扫描录入的子项条码为明细条码，则后续扫描的子项条码也必需为明细条码。
        List<TransScanInfo> list = ThreadTask.getTwo(() -> DatabaseProvider.getInstance().getTransScanInfoDao()
    ...
    public void setInBarcode(Context context, String internalCodeId) {
        List<TransScanInfo> list = ThreadTask.getTwo(() ->
                DatabaseProvider.getInstance().getTransScanInfoDao().queryByCodeAndType(internalCodeId, "2"));
        if (list.size() > 0) {
            mView.setInBarcode(list.get(0).getInBarcode());
        } else {
    ...
    public void rePacking(Context context, String setId, String tempId) {
        List<TransScanInfo> list = ThreadTask.getTwo(() ->
                DatabaseProvider.getInstance().getTransScanInfoDao().queryByCodeAndTypeIsNull(setId));
        if (list.size() == 0) {
        ...
        try {
            ...
            for (TransScanInfo transScanInfo : list) {
                if (transScanInfo.getOutBarcode().equals(outBarcode)) continue;
                BoxPackingsBean boxPackingsBean = new BoxPackingsBean();
                boxPackingsBean.setPackCode(transScanInfo.getOutBarcode());

                outBarcode = transScanInfo.getOutBarcode();
                List<BoxDetailsBean> boxDetailsBeanList = new ArrayList<>();

                List<TransScanInfo> outList = ThreadTask.getTwo(() ->
                        DatabaseProvider.getInstance().getTransScanInfoDao().queryByCodeAndTypeIsNull(setId));

                for (TransScanInfo u : outList) {
